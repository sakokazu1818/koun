...
function zipaddr_conf(){
	if( !empty($_GET['page']) && $_GET['page'] == zipaddr_KEYS ){
		if( isset($_POST['zipaddr_token']) && !empty($_POST['zipaddr_token']) ){
			...
			if( $sys_site < "1" || "4" < $sys_site )    $array[zipaddr_SYS.'site']= "4";
		...
		$sit= array("4" => "商用版サイト（default）","2" => "有償版サイト","3" => "御社サイト内で郵便番号簿管理");
...
　有償版サイト： <?php zipaddr_e(zipaddr2) ?> 系<br />
　御社サイト版： <?php zipaddr_e(zipaddr3COM) ?> 系<br />
<br />
※有償版は利用申請をしないと動きません。<br />
▼有償版のご利用には別途、<a href="<?php zipaddr_e(zipaddr2) ?>use/" target="_blank">利用申請（有償）</a> が必要となります。<br />
▼御社サイト版のご利用には別途、<a href="<?php zipaddr_e(zipaddr3sCOM) ?>use/" target="_blank">利用申請（有償）</a> が必要となります。<br />
▼[システム拡張AP識別子（'_'区切り）]<br />
　複数指定の例：WooCommerce_TrustForm<br />
<div class="btn-area">
	<ul><li>
