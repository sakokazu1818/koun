...
function zipaddr_jp_change($output, $opt=""){
	...
	$ninja= strstr($output,'ninja-forms-');
	if( strstr($output,'zip')==true || strstr($output,'postc')==true || $sys_drct!="" || !empty($ninja) ){;} //kword
	else  return $output;
...
if(isset($_SERVER['HTTPS'])) {$http=(empty($_SERVER['HTTPS'])||$_SERVER['HTTPS']=='off')? 'http':'https';}
	...
	$js.= empty($ninja) ? $pre."wp='1';" : $pre."wp='2';";
	...
	else if( !empty($ninja) )     $ans= $output.$js; //NinjaForms
